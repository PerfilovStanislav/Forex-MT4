...
#property copyright                       "Perfilov Family © (2016)"
#property icon                            "Bookmark.ico"
#property version                         "16.12"
...
bool OptimizeLots() {
   ...
   if(IsOptimization()) {   
      if (Optimization != 0) tempLots += MathFloor((TimeCurrent() - D'2015.01.15 00:00')/3600.0/24/Optimization)*0.01;
   }
...
bool checkIma_1_2_3(int cmd) {  
   ...
   double ima3    = iMA(NULL, 0, ma2_period, 0, int(ma2_method), ma2_price, 0);
   if (ma3_period == 0) {   
      if          (cmd == OP_SELL   &&  ima1 < ima3)  return (true);
      else if     (cmd == OP_BUY    &&  ima1 > ima3)  return (true);
      else                                            return (false);
   }
      
   double ima2     = iMA(NULL, 0, ma3_period, 0, int(ma3_method), ma3_price, 0);
   if             (cmd == OP_SELL   &&  ima1 < ima2   &&  ima2 < ima3)  return (true);
   else if        (cmd == OP_BUY    &&  ima1 > ima2   &&  ima2 > ima3)  return (true);   
